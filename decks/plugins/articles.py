...
STEPS = (
    "NewWords",            # 0  показали блок и спросили тот же блок
    "ShownWithExample",    # 1  явный пример рядом: `Das Haus ist weiß`
    "ShownWithHint",       # 2  косвенный пример или суффикс: род закодирован
    "GroupOfKnownWords",   # 3  дан артикль, выбрать слово; соседи знакомы
    "GroupOfShakyWords",   # 4  то же, но соседи сами шатаются
    "ChoiceOfTwo",         # 5  один неверный артикль погашен заранее
    "ChoiceOfThree",       # 6  der / die / das — то, чем колода была всегда
)
...
def material(card, step, ctx=None):
    ...
    if step == "NewWords":
        return {}
    if step == "ShownWithExample":
        if card.get("example"):
            return {"prop": card["example"], "prop_ru": card.get("example_ru", "")}
        return None
    if step == "ShownWithHint":
        if card.get("oblique"):
...
def render(card, easier=False, rung=None, intro=False, ctx=None):
    ...
    пример. Показ — половина ступени `NewWords`, вторая её половина это вопрос
    по тем же словам через целый блок других (`engine/scheduler.py`). Оценки у
    показа нет: спрашивать то, что стоит на экране, значит спрашивать умение
    читать.
...
def _forward(card, muted=False, prop="", prop_ru=""):
    ...
    `prop` — опора ступени: пример или строка про окончание. Она стоит **до**
    ответа, а не в отклике, и в этом вся ступень: у `ShownWithExample` ответ
    из строки вычитывается, у `ShownWithHint` — выводится.
    """
    input_spec = {
        "type": "choices",
